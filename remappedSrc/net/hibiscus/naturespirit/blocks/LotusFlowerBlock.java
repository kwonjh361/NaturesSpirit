...
public class LotusFlowerBlock extends BushBlock implements BonemealableBlock {
  ...
  public void entityInside(BlockState state, Level world, BlockPos pos, Entity entity) {
    ...
    if (!world.isClientSide) {
      ...
      if (blockstate.is(NSMiscBlocks.LOTUS_STEM) && isEntityAbove(pos, entity) && !isPowered(world, pos)) {
  ...
  private int getRedstonePower(LevelReader world, BlockPos pos) {
    ...
    } while (blockState.is(NSMiscBlocks.LOTUS_STEM));
  ...
  public void tick(BlockState state, ServerLevel world, BlockPos pos, RandomSource random) {
    ...
    BlockState blockstate = world.getBlockState(pos.below());
    if (blockstate.is(NSMiscBlocks.LOTUS_STEM) && isEntityAbove(pos, world.getNearestPlayer(pos.getX(), pos.getY(), pos.getZ(), 1.25D, false)) && !isPowered(world, pos)) {
  ...
  public void performBonemeal(ServerLevel world, RandomSource random, BlockPos pos, BlockState state) {
    ...
    if (world.getBlockState(pos.below()).is(NSMiscBlocks.LOTUS_STEM)) {
      ...
      world.setBlockAndUpdate(pos, NSMiscBlocks.LOTUS_STEM.defaultBlockState().setValue(LotusStemBlock.WATERLOGGED, false));
