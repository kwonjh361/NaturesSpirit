...
public class LotusPlantFeature extends Feature<NoneFeatureConfiguration> {
  ...
  public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
    ...
    if (structureWorldAccess.getBlockState(blockPos2).is(Blocks.WATER) || structureWorldAccess.isEmptyBlock(blockPos2)) {
      BlockState blockState = NSMiscBlocks.LOTUS_FLOWER.defaultBlockState();
      BlockState blockState2 = NSMiscBlocks.LOTUS_STEM.defaultBlockState().setValue(LotusStemBlock.WATERLOGGED, structureWorldAccess.getFluidState(blockPos2).is(FluidTags.WATER));
      ...
      for (int l = 0; l <= k; ++l) {
        if (structureWorldAccess.getBlockState(blockPos2).is(Blocks.WATER) && blockState2.canSurvive(structureWorldAccess, blockPos2)) {
          ...
          if (blockState.canSurvive(structureWorldAccess, blockPos2) && structureWorldAccess.getBlockState(blockPos2.below()).is(NSMiscBlocks.LOTUS_STEM)) {
