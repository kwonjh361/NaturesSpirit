  ...
  protected static void generateSquare(LevelAccessor world, BlockPos cornerPos, int radius, Direction direction1, Direction direction2) {
    ...
    for (int j = 0; j <= radius; ++j) {
      for (int k = 0; k <= radius; ++k) {
        ...
        if (world.isEmptyBlock(blockPos) || world.getBlockState(blockPos).is(NSMiscBlocks.GRAY_POLYPORE)) {
          world.setBlock(blockPos, NSMiscBlocks.GRAY_POLYPORE_BLOCK.defaultBlockState().setValue(HugeMushroomBlock.DOWN, false), 2);
        }
      }
    }
